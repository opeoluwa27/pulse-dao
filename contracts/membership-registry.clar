...
(map-set role-weights { role: ROLE-PATIENT } { weight: u1 })
(map-set role-weights { role: ROLE-PROVIDER } { weight: u2 })
(map-set role-weights { role: ROLE-ADMIN } { weight: u3 })
...
(define-private (is-authorized-admin)
  (let ((caller-info (default-to 
                      { role: u0, status: u0, joined-at: u0, expires-at: u0, verified-by: none } 
...
(define-read-only (get-member-weight (address principal))
  (let ((member-info (default-to 
    ...
    (if (is-active-member address)
      (get weight (default-to { weight: u0 } (map-get? role-weights { role: (get role member-info) })))
      u0
    )
  )
)
...
(define-public (apply-for-membership (role uint) (credentials (string-utf8 256)))
  (begin
    ...
    (asserts! (is-none (map-get? applications { applicant: tx-sender })) ERR-ALREADY-APPLIED)
...
(define-public (approve-membership (applicant principal) (membership-duration uint))
  (let (
    (application (map-get? applications { applicant: applicant }))
  )
...
(define-public (reject-membership (applicant principal))
  (begin
    ...
    ;; Check if application exists
    (asserts! (is-some (map-get? applications { applicant: applicant })) ERR-APPLICATION-NOT-FOUND)
...
;; Renew membership
(define-public (renew-membership (address principal) (extension-period uint))
...
(define-public (terminate-membership (address principal))
  (let ((member-info (map-get? memberships { address: address })))
    ...
    (asserts! (or (is-eq tx-sender address) (is-authorized-admin)) ERR-NOT-AUTHORIZED)
...
;; Update member role
(define-public (update-member-role (address principal) (new-role uint))
  (let ((member-info (map-get? memberships { address: address })))
    ;; Check admin authorization
    (asserts! (is-authorized-admin) ERR-NOT-ADMIN)
...
(define-public (update-role-weight (role uint) (weight uint))
  (begin
    ;; Check admin authorization
    (asserts! (is-authorized-admin) ERR-NOT-ADMIN)
