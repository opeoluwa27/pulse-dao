...
(define-private (record-transaction (tx-type (string-ascii 20)) (amount uint) (recipient principal) (description (string-ascii 256)))
  (let
...
(define-private (transfer-funds (amount uint) (is-emergency bool))
  (if is-emergency
    (begin
    ...
    (begin
      (asserts! (<= amount (- (var-get treasury-balance) (var-get emergency-fund-balance))) ERR-INSUFFICIENT-FUNDS)
...
(define-public (allocate-to-emergency-fund (amount uint))
  (begin
    ...
    ;; Record the transaction
    (record-transaction "emergency-alloc" amount tx-sender "Emergency fund allocation")
    (ok true)
  )
)

;; Register an approved proposal for funding
(define-public (register-approved-proposal (amount uint) (recipient principal) (description (string-ascii 256)))
  (let
...
;; Execute funding for an approved proposal
(define-public (execute-proposal-funding (proposal-id uint))
  (let
    ((proposal (unwrap! (map-get? approved-proposals { proposal-id: proposal-id }) ERR-PROPOSAL-NOT-FOUND)))
    
    (asserts! (is-authorized) ERR-NOT-AUTHORIZED)
    (asserts! (not (get executed proposal)) ERR-PROPOSAL-ALREADY-PAID)
    
    ;; Transfer funds
    (unwrap! (transfer-funds (get amount proposal) false) ERR-INSUFFICIENT-FUNDS)
    ...
    ;; Record the transaction
    (record-transaction "proposal-funding" 
                       (get amount proposal) 
...
(define-public (register-recurring-expense (recipient principal) (amount uint) (frequency uint) (description (string-ascii 256)))
  (let
...
;; Process a recurring expense payment
(define-public (process-recurring-expense (expense-id uint))
  (let
    ((expense (unwrap! (map-get? recurring-expenses { expense-id: expense-id }) ERR-EXPENSE-NOT-FOUND))
     (current-height (unwrap-panic (get-block-info? height u0))))
    
    (asserts! (is-authorized) ERR-NOT-AUTHORIZED)
    (asserts! (get active expense) ERR-EXPENSE-NOT-FOUND)
    (asserts! (>= current-height (+ (get last-paid-height expense) (get frequency expense))) ERR-EXPENSE-ALREADY-PAID)
    
    ;; Transfer funds
    (unwrap! (transfer-funds (get amount expense) false) ERR-INSUFFICIENT-FUNDS)
    
    ;; Update last paid height
    (map-set recurring-expenses
      { expense-id: expense-id }
      (merge expense { last-paid-height: current-height })
    )
    
    ;; Record the transaction
    (record-transaction "recurring-expense" 
                       (get amount expense) 
                       (get recipient expense) 
                       (get description expense))
...
(define-public (create-emergency-request (amount uint) (description (string-ascii 256)))
  (let
...
(define-public (approve-emergency-request (request-id uint))
  (let
    ((request (unwrap! (map-get? emergency-requests { request-id: request-id }) ERR-EMERGENCY-REQUEST-NOT-FOUND))
     (current-approvers (get approvers request)))
    
    (asserts! (is-authorized) ERR-NOT-AUTHORIZED)
    (asserts! (not (get executed request)) ERR-PROPOSAL-ALREADY-PAID)
    (asserts! (not (is-some (index-of current-approvers tx-sender))) ERR-INVALID-PARAMETER)
...
(define-public (execute-emergency-request (request-id uint))
  (let
    ...
    ;; Record the transaction
    (record-transaction "emergency-fund" 
                       (get amount request) 
